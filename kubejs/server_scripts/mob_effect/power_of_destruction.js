...
function powerOfDestructionPlayerHurtByOthers(event, data) {
    ...
    if (entity.isLiving() && entity.hasEffect('kubejs:power_of_destruction'))
    {
         if(player.getArmorValue() >= 20)
         {
             ...
             event.amount = event.amount + Math.floor(player.getMaxHealth()*0.5*amplifier)
         }
         else if(0 < player.getArmorValue() < 20)
         {
            ...
            event.amount = event.amount + Math.floor(player.getMaxHealth()*1.5*amplifier)
         }
         else if(player.getArmorValue() <= 0)
         {
            ...
            event.amount = event.amount + Math.floor(player.getMaxHealth()*2.5*amplifier)
         }
    }
