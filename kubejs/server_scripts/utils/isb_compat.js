...
function overLimitSpellCast(resourceLocation, amplifier, player, consume) {
    ...
    if (itemMap.has('kubejs:plastic_heart') && typeMap.has('kubejs:magic')) {
        amplifier = amplifier + typeMap.get('kubejs:magic')
    }
    $SpellRegistry["getSpell(net.minecraft.resources.ResourceLocation)"](resourceLocation).attemptInitiateCast(Item.of('air'), amplifier, player.level, player, $CastSource.NONE, consume)
}
